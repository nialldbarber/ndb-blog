...
A good use case for this is caching values, or memoization. Memoization is the technique of storing the value of an expensive function call, memorising it and if the inputs to the function are the same, return the stored value instead of running the expensive functin again.
...
Next we create an empty object where we will store our `func` parameters as keys and returned values as values. Once called again, if the parameters are the same, the cache function will return the value of whatevers in `cacheObj`.
