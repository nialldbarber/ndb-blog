---
title: Array method mental models
date: '2022-01-02'
tags: ['javascript', 'mental models']
draft: false
...
[`Array.prototype.filter()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter) is a method that takes a callback as it's argument and returns a new array based on the callbacks logic. It also takes two other arguments: the current index and the array you are mapping over.
...
[`Array.prototype.map()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/map) is a method that takes a callback as it's first argument, which does something to each element of the array. It also has two more arguments: index and the array.
...
The most common example for `.reduce()` is the find the total sum of an array:
...
In the example above we start with an initial value of an empty object. We then loop over the `employees` array and spread each new item into the eventual return value of `employeesObj`. From that we are using the employees name as the key for the new object.
...
This mental model is a sand castle; a final piece that is made from many pieces. A final product, if you will. The castle acts as the final returned value and also the accumlator value. Each piece of sand you add to the castle acts as the current value that you're adding to the total value.
